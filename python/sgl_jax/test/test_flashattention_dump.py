...
class TestAttention(CustomTestCase):
    ...
    def run_test_gpu(self, mode, lens, mode_args, qkv_path='', **mode_kwargs):
        ...
        expected = ref_ragged_paged_attention(
            ...
            #**mode_kwargs, # Note(Lin): no need to pass mode_kwargs on this branch since temperature feat is not avialable
        )
        ...
        if qkv_path:
            ...
            print(f"{qkv_path} saved!", np.isnan(q_fp32).any(), np.isnan(k_fp32).any(), np.isnan(v_fp32).any(), np.isnan(fa_fp32).any(), np.isnan(naive_fp32).any())
            return
